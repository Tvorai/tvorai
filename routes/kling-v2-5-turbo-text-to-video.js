/**
 * Kompletný route: Kling V2.5 Turbo Text to Video (T2V)
...
router.post('/generate', async (req, res) => {
  try {
    ...
    if (!prompt || typeof prompt !== 'string' || !prompt.trim()) {
      return res.status(400).json({ ok: false, error: "Missing or empty 'prompt'" });
    }
    ...
    console.log('✅ Novita API response:', response.data);

    const generationId = response.data?.generationId;
    if (!generationId) {
      console.error('❌ generationId missing! Full response:', response.data);
      return res.status(500).json({ ok: false, error: 'NO_GENERATION_ID_FROM_NOVITA' });
    }

    return res.json({ ok: true, generationId });
  } catch (err) {
    console.error('❌ /generate error:', err.message);
    return res.status(500).json({ ok: false, error: 'Server error during generation' });
  }
});
